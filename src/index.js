...
button2.addEventListener("click", () => {
  ...
  function generateECGCoordinates(numPoints, amplitude, frequency, phaseShift) {
    ...
    for (let i = 0; i < numPoints; i++) {
      const x = i;
      const y = amplitude * Math.sin((2 * Math.PI * frequency * x) / numPoints + phaseShift);
      coordinates.push({ x, y });
    }
    return coordinates;
  }

  const numPoints = 1000; // Número de puntos para el ECG
  const amplitude = 185; // Amplitud de la onda
